function CtdCorrDataFile = TSdiags_from_Struct(withARGO,ARGOCorr,TESTdat,depNUM_ARGO, CtdCorrDataFile, CTD)
%
%
...
%
% INPUT:
    % withARGO == 0 (no)  - just plots CTD data.
    % withARGO == 1 (yes) - plots CTD data, then, on top, plots ARGO
        % data that will be corrected in whitespace correction code (filename ARGO_SalinityCorrection_TSdiag_WhitespaceMaximisation.m)
    % ARGOCorr == 0 (no)  - plot uncorrected ARGO data  on top
    % ARGOCorr == 1 (yes) - plot uncorrected, then corrected ARGO data on top
    % TESTdat               - if withARGO == 1, input test data  is the ARGO data
    % depNUM_ARGO         - the deployment number of the ARGO campaign,
...
if length(CtdCorrDataFile.path) == 2
...
if length(CtdCorrDataFile.path) == 3 % and which of these cruises should be on the very top:
...
% legend name:
for n = 1:length(s)
    %hh.(char(s{n}))= plot(reshape(CTD.Corrected.SALT_01.*CTD.Corrected.FLAG_01,[],1),reshape(CTD.Corrected.ptemp_01,[],1),'.','markersize',5,'color',CC(n,:));
    hh.(char(s{n}))= plot(reshape(CTD.Corrected.SALT_01.*CTD.Corrected.FLAG_01,[],1),reshape(CTD.Corrected.ptemp_01,[],1),'.','markersize',5,'color','red');
    LEG{n+1,1} = s{n};     
    hold on
end

% if plotting ARGO data, plot this over the top in grey, add to the legend:
if withARGO == 1
    hh.TEST = plot(TESTdat.S,TESTdat.PT,'.','markersize',5,'color','k');
    LEG = [LEG;'ARGO float TEST']; 
    if ARGOCorr == 1
        hh.TESTcorr = plot(TESTdat.Corr.S,TESTdat.Corr.PT,'.','markersize',5,'color','b');
        LEG = [LEG; ' ARGO float TEST corrected']; 
    end
end
grid on
% plot legend:
...
% option to have cruise ontop of ARGO (useful if the ARGO has more datapoints and "hides" the underlying background data):
if withARGO == 1
    AGAIN = questdlg('Plot with a comparison cruise on top of ARGO?','stack data','yes','no','no');
...
if withARGO == 0
    ...
    fnameOut = strcat(depNUM_ARGO,'_TSdiag_halfmCORR_',num2str(length(DIR)-2+1),'_',AXES,'_',KEYWORD);
