function [area] = imageArea_V2(A,  condARGO, tempCondCellARGO, pressureARGO, ~, salinityShip, ptempShip, imageDir,AXISlims,counter)
%
% imageArea_V2: Computes the whitespace area on a figure of a TS diagram
% with data points from test ARGO data and background ship data.
%
%  Syntax:
%    [area] = imageArea_V2(A,  condARGO, tempCondCellARGO, pressureARGO, ptempARGO, salinityShip, ptempShip, imageDir,AXISlims,counter)
%
%
%
% Code Description: The conductivity correction coefficient A is applied to
% the input test conductivity (condARGO) and an adjusted salinity and
...
%        2.  condARGO          = test (ARGO) data conductivity
%        3.  tempCondCellARGO = test (ARGO) data in situ temperature;
%        4.  pressureARGO     = test (ARGO) pressure
%        5.  ptempARGO        = test (ARGO) data potential temperature
...
if exist('GSW','dir')
    salinityARGOAdj   = gsw_SP_from_C(condARGOAdj,tempCondCellARGO, pressureARGO);
    ptempARGOAdj      = sw_ptmp(salinityARGOAdj,tempCondCellARGO, pressureARGO,0);
elseif exist('seawater','dir')
    ...
    salinityARGOAdj   = sw_salt(condARGOAdj * (10 / sw_c3515()), tempCondCellARGO, pressureARGO);
    ptempARGOAdj      = sw_ptmp(salinityARGOAdj,tempCondCellARGO, pressureARGO,0);
else
    msgbox('NEED TO DOWNLOAD SEAWATER PACKAGE')
end

%  Create T/S diagram of test (ARGO) data against background (ship)
%  data:
markerSize = 10;
scrsz = get(0,'ScreenSize');
fig   = figure('Position',[50 50 scrsz(3)/2 scrsz(4)-200]);
plot(salinityARGOAdj,ptempARGOAdj,'b.','markersize', markerSize);
hold on
% grid on
...
% legend('ARGO data (blue)','Background (ship) data (red)','location','best')
...
% saveas(fig,'/home/cmunoz/test.png')
% image='/home/cmunoz/test.png'
% I = imread(image);
% level = graythresh(I)
% BWAdj  =im2bw(I,level);
